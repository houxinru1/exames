 ...
 *  @dparam payload    pageNum    : <number>,     //非必填  想获取第几页的数据  如果不传 默认是第1页；
 ***/
 export const getpermissionlistApi = function (payload = {}) {
  return axios.post("/permission/list", payload, getPostConfig());
};

/**
 * @description  创建权限接口
 *  *   @dparam payload    title :<string>, //标题
      * @dparam payload    type  :<number>  //类型   1代表左侧栏  2代表页面  3代表功能
     *  @dparam payload     pid   :<number>  //pid如果为空就是最大的一级  如果等于某项的id就是它的子级
 ***/
 export const getpermissioncreateApi = function (payload = {}) {
...
/**
 * @description  修改权限接口
        *  *   @dparam payload    id   : <number>     //id
        *  *   @dparam payload    title: <string>,    //标题
        *  *   @dparam payload    type : <number>,    //类型
        *  *   @dparam payload    pid  : <number>,    //父id
 ***/
 export const getpermissionupdateApi = function (payload = {}) {
...
/**
 * @description  删除权限接口
 *  *   @dparam payload    id: <arr>,   // 必填项 权限的id  如果有子级 就把子级跟父级的id都传过来 是一个数组
 ***/
